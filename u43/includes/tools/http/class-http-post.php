...
class HTTP_Post extends Tool_Base {
    ...
    public function execute($inputs, $context = []) {
        ...
        // Get request body
        $body = isset($inputs['body']) ? $inputs['body'] : null;
        // Filter out internal meta data and empty pair markers if body is an array
        if (is_array($body)) {
            unset($body['__pairs_meta__']);
        ...
        $body = $this->resolve_variables_in_array($body, $context);
        
        // Convert body to JSON string if it's an array or object
        $body_string = null;
        if ($body !== null) {
            if (is_string($body)) {
                ...
                if (json_last_error() === JSON_ERROR_NONE) {
                    ...
                    // Not JSON, try to wrap it or use as is
                    $body_string = $body;
                }
        ...
        $response = $this->make_request('POST', $url, $headers, $body_string);
        ...
        foreach ($parts as $part) {
            ...
            if (preg_match('/^(.+)\[(\d+)\]$/', $part, $matches)) {
                ...
                if (is_array($value) && isset($value[$key]) && is_array($value[$key])) {
                    $value = isset($value[$key][$index]) ? $value[$key][$index] : null;
                } else {
                    return null;
                }
            } else {
